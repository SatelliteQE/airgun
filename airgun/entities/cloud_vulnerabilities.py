...
class CloudVulnerabilityEntity(BaseEntity):
    ...
    def validate_cve_to_host_details_flow(self, cve_id, hostname=None):
        ...
        cve_details_view.browser.element(
            f'.//a[contains(text(), "{hostname}")]'
        ).click()
        host_details_view = NewHostDetailsView(self.browser)
        host_details_view.breadcrumb.wait_displayed()
        wait_for(
            lambda: host_details_view.vulnerabilities.vulnerabilities_table.is_displayed,
            timeout=30,
        )
        vulnerabilities = getattr(
            host_details_view.vulnerabilities, "vulnerabilities_table", None
        )
