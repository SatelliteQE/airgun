...
class ContentViewFilterEntity(BaseEntity):
    ...
    def update_package_rule(self, cv_name, filter_name, rpm_name, new_values,
        ...
        row[ACTIONS_COLUMN].widget.save.click()
        view.flash.assert_no_error()
        view.flash.dismiss()

    def search_package_rule(self, cv_name, filter_name, query):
    ...
    def add_module_stream(
            self, cv_name, filter_name, module_stream):
        """Add module stream to module stream content view filter.

        :param str cv_name: content view name
        :param str filter_name: content view filter name
        :param str module_stream: module stream name
        """
        view = self.navigate_to(
            self, 'Edit',
            cv_name=cv_name,
        ...
        view.content_tabs.add(module_stream)
        view.flash.assert_no_error()
        view.flash.dismiss()
...
class EditContentViewFilter(NavigateStep):
    ...
    def resetter(self, *args, **kwargs):
        ...
        added/updated package filter rows otherwise.
        """
        self.obj.browser.refresh()
