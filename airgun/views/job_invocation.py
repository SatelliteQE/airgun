    ...
    def is_displayed(self):
        ...
        try:
            ...
            if not locations or locations[0] != 'Jobs':
                # Fallback: check if wizard elements are present
                # If category_and_template step is visible, we're on the right page
                return self.category_and_template.job_category.is_displayed
        except (AttributeError, NoSuchElementException):
            # If breadcrumb parsing fails, check for wizard elements
            try:
                return self.category_and_template.job_category.is_displayed
            except (AttributeError, NoSuchElementException):
                return False
        ...
        try:
            ...
            # Fallback to checking wizard elements
            try:
                return self.category_and_template.job_category.is_displayed
            except (AttributeError, NoSuchElementException):
                return False
